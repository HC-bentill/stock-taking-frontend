...
const School = ( { match } ) => {
  ...
  const history = useHistory();
    ...
    useEffect( () => {
        axios
        .get(`http://localhost:1337/api/schools/${match.params.id}`)
        .then(response => {
          // Handle success.
          ...
          //     delete new_obj['publishedAt']
          //     delete new_obj['publishedAt']

          //   return [...arr, new_obj]
          // }, [])
          setSchoolData( data.attributes );
          console.log('particular school data:', schoolData);
        })
        .catch(error => {
          // Handle error.
          swal({
    ...
    // const school = schoolData.find( school => school.id.toString() === match.params.id)
    // const schoolDetails = school ? Object.entries( school ) :
  ...
  return (
        ...
        <CCard className="p-4">
          ...
          <CCardBody>
              <table className="table table-hover">
                <tbody>
                  <tr>
                    <td><strong>name: </strong>{schoolData.name} </td>
                  </tr>
                  <tr>
                    <td><strong>Location: </strong> {schoolData.location} </td>
                  </tr>
                  <tr>
                    <td><strong>Contact: </strong> {schoolData.contact} </td>
                  </tr>
                </tbody>
              </table>   
            </CCardBody>
            <CRow>
                <CCol md={6}>
                    <CButton onClick={() => setEditModal(!editModal)} className="" block color="primary" aria-pressed="true">
                        <CIcon size="sm" name="cil-pencil" />
                        <span className="mfs-2">Edit</span>
                    </CButton>
                </CCol>
                <CCol md={6}>
                    <CButton onClick={() => setDeleteModal(!deleteModal)} className="" block color="danger" aria-pressed="true">
                        <CIcon size="sm" name="cil-pencil" />
    ...
    >
      <CModalHeader closeButton>
        <CModalTitle><strong>Warning !</strong></CModalTitle>
        </CModalHeader>
        <CModalBody>
          <strong>Are you sure you want to delete this record ?</strong>
        </CModalBody>
        <CModalFooter>
          <CButton color="danger" onClick={() => handleDelete()}>Confrim</CButton>{' '}
    ...
    >
      <CModalHeader closeButton>
        <CModalTitle><strong>Edit School</strong></CModalTitle>
      </CModalHeader>
      <CModalBody className="p-4">
        <CForm onSubmit={handleEdit} formNoValidate>
          ...
          <CInputGroup className="mb-3">
            ...
            <CButton onClick={() => handleEdit()} type="submit" color="primary">Save</CButton>{' '}
